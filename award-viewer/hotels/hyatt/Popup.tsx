...
function HyattPopup({ onBack, site }: HyattPopupProps) {
  const [valueSettings, setValueSettings] = useState<HyattValueSettings>(
    DEFAULT_HYATT_VALUE_SETTINGS
  )
  ...
  useEffect(() => {
    const loadSettings = async () => {
      if (!chrome?.storage?.local) {
        setValueSettings(DEFAULT_HYATT_VALUE_SETTINGS)
        return
      }
      ...
      setValueSettings(
        normalizeHyattValueSettings(
          stored[HYATT_VALUE_SETTINGS_KEY] as
            | Partial<HyattValueSettings>
            | undefined
        )
      )
    }
  ...
  const updateSetting = async (
    key: keyof HyattValueSettings,
    value: number
  ) => {
    const nextSettings = normalizeHyattValueSettings({
      ...valueSettings,
      [key]: value
    })
    ...
    await chrome.storage.local.set({
      [HYATT_VALUE_SETTINGS_KEY]: nextSettings
    })
  }
      ...
      <div
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
          ...
          <p
            ...
            Highlight values on Hyatt search results automatically when the
