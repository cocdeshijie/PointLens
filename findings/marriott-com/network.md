...
### Browser-managed XHR headers, 2026-09-06

- After PR #39, the user's Chrome recorded unsafe-header warnings attributed to PointLens's `XMLHttpRequest.setRequestHeader` wrapper on `/default.mi`: `Cookie` (both cases), `user-agent`, `sec-fetch-*`, and `referer`. The observer forwarded these attempted headers even though Chrome rejects them.
- The observer now skips those manual browser-managed headers before capture or forwarding. Session cookies still travel through the browser's normal credentials handling; allowed application headers retain native validation and are captured only after the native setter succeeds. Cookie/referrer/security-header ownership follows https://fetch.spec.whatwg.org/#forbidden-request-header; the user-agent guard also covers the observed Chrome XHR warning.
- Added a production-extension Chromium regression for both observed pricing XHR and unrelated XHR. It asserts no unsafe-header console errors, real browser cookies instead of attempted overrides, preserved authorization/content-type/request-ID headers, successful responses, and no additional requests. Production build, type checking, 18 unit tests, and 29 browser tests pass.
- Cleared the historical errors and reloaded PointLens in the user's Chrome. Opened Marriott's home page and submitted its saved Chicago Oct 12–13 search. The results rendered CPP badges (Residence Inn Downtown/Loop: 1.57¢/pt and 54,000 points), and the extension error list remained empty.
